...
class PEArray2D(pe_size: (Int, Int), vec: Seq[Int], width: Seq[Int], addr_width: Int = 16,stt: DenseMatrix[Int], access: Seq[DenseMatrix[Int]], io_type: Array[Boolean], latency: Int,  op_type: Int = 0, time_range: Seq[Array[Int]], mem_dims: Seq[Array[Int]], mem_size: Seq[Int], back: Boolean) extends Module{
  // val dims = 1 +: (0 until time_range.length-1).map(x=>time_range.slice(0, x+1).reduce(_*_)).toArray
  val time_ctrl = Module(new MultiDimTime(addr_width, time_range(1), Array.fill(time_range(1).length)(0))).io
  ...
  val mem = for(i <- 0 until num_op) yield{
    for(j <- 0 until bank_peid(i).length) yield{
      ...
      if(dataflows(i)==StationaryDataflow)
        Module(new MemController(mem_size(i), width(i) * vec(i), addr_width, mem_dims(i), mem_dims(i), time_range(i), time_range(i), init_rd_idx, init_wr_idx)).io
      else
        Module(new MemController(mem_size(i), width(i) * vec(i), addr_width, mem_dims(i), mem_dims(i), time_range(i), time_range(i), init_rd_idx, init_wr_idx)).io
    }
  }
