...
class PEArray_Mem extends Module{
  ...
  val mat1_mem = Seq.fill(pe_size._1)(Module(new MultiDimMem(mem_dim, mem_size, simd_width(0))).io)
  val mat2_mem = Seq.fill(pe_size._2)(Module(new MultiDimMem(mem_dim, mem_size, simd_width(1))).io)
  val matout_mem = Seq.fill(pe_size._1)(Module(new MultiDimMem(mem_out_dim, mem_out_size, simd_width(2))).io)
  ...
  for(i <- 0 until pe_size._1){
    ...
    matout_mem(i).rd_addr.valid := true.B
    matout_mem(i).rd_addr.bits := matout_rd_time(i).out
    matout_rd_time(i).in := true.B
